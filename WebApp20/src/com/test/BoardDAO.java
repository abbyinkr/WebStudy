...
public class BoardDAO
{
	...
	// 생성자 정의 -- 인스턴스 생성시 매개변수 넘겨주는 방식으로 구성
	...
	*/
	...
	public int getDataCount(String searchKey, String searchValue)
	{
		...
		try
		{
			// 검색키워드 전처리 - 앞 뒤로 와일드카드 % 붙여주기
			searchValue = "%" + searchValue + "%";
			
			// 쿼리문 띄어쓰기 주의~!! (WHERE 구문 추가)
			sql = "SELECT COUNT(*) AS COUNT FROM TBL_BOARD";
	...
	*/
	...
	public int getBeforeNum(int num)
	{
		...
		try
		{
			// 현재페이지 이전의 게시물 번호중에 가장 큰 값 = 직전의 게시물 번호
	...
	public int getNextNum(int num)
	{
		...
		try
		{
			// 현재페이지 이후의 게시물번호중에 가장 작은 값 = 직후의 게시물 번호
